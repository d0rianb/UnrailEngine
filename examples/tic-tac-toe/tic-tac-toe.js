...
const grid = new ft(3, 3);
const game = new mt("Tic Tac Toe");
let line = null;
let lineAnimation = new Ct(0, 1, 1e3, "smoothStep");
function cross(x, y) {
  ...
  it.line(rect.x, rect.y, rect.x2, rect.y2);
  it.line(rect.x2, rect.y, rect.x, rect.y2);
}
function checkWins() {
  for (let i = 0; i <= 2; i++) {
...
function render() {
  ...
  for (let cell of grid.cells) {
    it.rect(cell.x * cellWidth, cell.y * cellWidth, cell.width * cellWidth, cell.height * cellWidth);
    cell.state && cell.state == "circle" && it.circle(cell.x * cellWidth + cellWidth / 2, cell.y * cellWidth + cellWidth / 2, cell.width * cellWidth / 3);
    cell.state && cell.state == "cross" && cross(cell.x * cellWidth, cell.y * cellWidth);
  }
  if (line)
    it.line(line.x, line.y, line.x + (line.x2 - line.x) * lineAnimation.value, line.y + (line.y2 - line.y) * lineAnimation.value, { strokeStyle: "red", lineWidth: 6 });
  it.endFrame();
}
function reset() {
  line = null;
