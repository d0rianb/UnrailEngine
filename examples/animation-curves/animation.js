...
class Renderer {
  ...
  static line(x1, y1, x2, y2, obj) {
    Renderer.style(obj);
    ctx.beginPath();
    ctx.moveTo(round(offset$1.x + x1), round(offset$1.y + y1));
    ctx.lineTo(round(offset$1.x + x2), round(offset$1.y + y2));
    ctx.stroke();
  }
  static rect(x, y, width, height, obj) {
    Renderer.style(obj);
    const [r_x, r_y, r_w, r_h] = [round(x + offset$1.x), round(y + offset$1.y), round(width), round(height)];
  ...
  static poly(points, obj) {
    ...
    Renderer.style(obj);
    ctx.beginPath();
    ctx.moveTo(round(points[0].x + offset$1.x), round(points[0].y + offset$1.y));
    for (let i = 1; i < points.length; i++) {
      ctx.lineTo(round(points[i].x + offset$1.x), round(points[i].y + offset$1.y));
    }
    ctx.fill();
    ctx.stroke();
  }
  static circle(x, y, radius, obj) {
    Renderer.style(obj);
    ctx.beginPath();
    ctx.arc(round(x + offset$1.x), round(y + offset$1.y), radius, 0, TWOPI);
    ctx.fill();
    ctx.stroke();
  }
  ...
  static rectSprite(x, y, width, height, texture) {
    ...
    Renderer.style({});
    ctx.save();
    ctx.translate(round(x + width / 2 + offset$1.x), round(y + height / 2 + offset$1.y));
...
let linear_x = new Animation(0, dimension, 2e3, Easing.linear, { autostart: true, loop: true });
let linear_y = new Animation(dimension, 0, 2e3, "linear", { autostart: true, loop: true });
let easeIn_y = new Animation(dimension, 0, 2e3, "easeIn", { autostart: true, loop: true });
let easeOut_y = new Animation(dimension, 0, 2e3, Easing.easeOut, { autostart: true, loop: true });
let easeInOut_y = new Animation(dimension, 0, 2e3, Easing.easeInOutBack, { autostart: true, loop: true });
...
function draw(ts) {
  ...
  Renderer.rect(offset, offset, dimension, dimension, { lineWidth: 0.5 });
  Renderer.circle(offset + linear_x.value, offset + linear_y.value, 0.5);
  Renderer.rect(dimension + 2 * offset, offset, dimension, dimension, { lineWidth: 0.5 });
  Renderer.circle(dimension + 2 * offset + linear_x.value, offset + easeIn_y.value, 1);
  Renderer.rect(2 * dimension + 3 * offset, offset, dimension, dimension, { lineWidth: 0.5 });
  Renderer.circle(2 * dimension + 3 * offset + linear_x.value, offset + easeOut_y.value, 1);
  Renderer.rect(3 * dimension + 4 * offset, offset, dimension, dimension, { lineWidth: 0.5 });
