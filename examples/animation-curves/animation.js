...
let linear_x = new Lt(0, dimension, 2e3, gt.linear, { autostart: true, loop: true });
...
let easeOut_y = new Lt(dimension, 0, 2e3, gt.easeOut, { autostart: true, loop: true });
let easeInOut_y = new Lt(dimension, 0, 2e3, gt.easeInOutBack, { autostart: true, loop: true });
let speed = new Lt(0, width - 2 * offset, 2500, gt.easeInOut, { autostart: true, loop: true });
let speed2 = new Lt(0, width - 2 * offset, 2500, gt.linear, { autostart: true, loop: true });
let speed3 = new Lt(0, width - 2 * offset, 2500, gt.easeIn, { autostart: true, loop: true });
function draw(ts) {
  E.clear();
  E.text("Linear", offset, offset + dimension + 20, { size: 16 });
  E.rect(offset, offset, dimension, dimension, { lineWidth: 0.5 });
  E.circle(offset + linear_x.value, offset + linear_y.value, 0.5);
  E.rect(dimension + 2 * offset, offset, dimension, dimension, { lineWidth: 0.5 });
  E.circle(dimension + 2 * offset + linear_x.value, offset + easeIn_y.value, 1);
  E.rect(2 * dimension + 3 * offset, offset, dimension, dimension, { lineWidth: 0.5 });
  E.circle(2 * dimension + 3 * offset + linear_x.value, offset + easeOut_y.value, 1);
  E.rect(3 * dimension + 4 * offset, offset, dimension, dimension, { lineWidth: 0.5 });
  E.circle(3 * dimension + 4 * offset + linear_x.value, offset + easeInOut_y.value, 1);
  E.circle(offset + speed.value, height / 2, 10);
  E.circle(offset + speed2.value, height / 2 + 25, 10);
  E.circle(offset + speed3.value, height / 2 + 50, 10);
}
let pause = false;
ot.addButton(() => pause ? "||" : ">", (e) => {
  pause = !pause;
  linear_x.toggle();
  linear_y.toggle();
});
ot.addButton(() => "Reset", (e) => {
  linear_x.reset();
  linear_y.reset();
});
const game = new ht("Animation Test");
E.create();
