...
class $ {
  constructor(t2, e2) {
    ...
    this.id = q++, this.image = new Image(), this.image.src = t2, this.size = new d(this.image.width, this.image.height), this.rotation = (e2 == null ? void 0 : e2.rotation) || 0, this.offset = (e2 == null ? void 0 : e2.offset) || u, this.scale = (e2 == null ? void 0 : e2.scale) || m;
  }
...
class bt extends mt {
  ...
  render() {
    ht.renderer.circle(this.pos.x, this.pos.y, this.radius, { fillStyle: this.color, lineWidth: 1, globalAlpha: this.opacity / 255 });
  }
}
class xt {
...
class Enemy extends mt {
  constructor(x, y) {
    ...
    this.width = 80;
    this.height = 50;
    this.texture = new $("resources/assets/InvaderA2.png");
  }
  isDead() {
    if (!this.alive)
      return;
...
class Player extends pt {
  constructor(x, y) {
    ...
    this.health = 100;
    this.alive = true;
    this.texture = new $("resources/assets/space-invader-player.png");
  }
  isDead() {
    this.alive = false;
  }
...
class Env extends ut {
  ...
  hit(shot, enemy) {
    ...
    const PG = new xt(50, new d(shot.x, shot.y), 700, () => {
...
ot.addItem(() => `Score : ${env.score}`, "top-left");
ot.addItem(() => `Health : ${env.player.health}`, "top-right");
ot.addButton(() => paused ? "||" : ">", (e) => paused = !paused);
const { width, height } = b();
st.create();
const env = new Env(width, height);
const game = new ht("Space Invader", env);
