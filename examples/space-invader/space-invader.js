...
class lt {
  constructor(t2, e2) {
    ...
    this.id = at++, this.image = new Image(), this.image.src = t2, this.image.onload = () => this.isLoaded = true, this.size = new h(this.image.width, this.image.height), this.rotation = (e2 == null ? void 0 : e2.rotation) || 0, this.offset = (e2 == null ? void 0 : e2.offset) || m, this.scale = (e2 == null ? void 0 : e2.scale) || p;
  }
...
class Gt extends bt {
  ...
  render() {
    pt.renderer.circle(this.pos.x, this.pos.y, this.radius, { fillStyle: this.color, lineWidth: 1, globalAlpha: this.opacity / 255 });
  }
}
class ft {
  constructor(t2, e2, s2, i2) {
...
class Enemy extends bt {
  constructor(x, y) {
    ...
    this.width = 80;
    this.height = 50;
    this.texture = new lt("resources/assets/InvaderA2.png");
  }
  isDead() {
    if (!this.alive)
      return;
  ...
  render() {
    if (!this.alive)
      return;
    nt.rectSprite(this.x, this.y, this.width, this.height, this.texture);
  }
}
class Player extends Zt {
  constructor(x, y) {
    super(x, y);
    this.health = 100;
    this.alive = true;
    this.texture = new lt("resources/assets/space-invader-player.png");
  }
  isDead() {
    this.alive = false;
  }
  ...
  render() {
    nt.rectSprite(this.x, this.y, 60, 30, this.texture);
  }
}
class Shot extends bt {
  constructor(x, y) {
    super(x, y);
    this.width = 2;
    this.height = 5;
  ...
  render() {
    nt.rect(this.x, this.y, this.width, this.height, { lineWidth: 4, strokeStyle: "red" });
  }
}
class Env extends yt {
  ...
  hit(shot, enemy) {
    ...
    const PG = new ft(50, new h(shot.x, shot.y), 700, () => {
...
rt.addItem(() => `Score : ${env.score}`, "top-left");
rt.addItem(() => `Health : ${env.player.health}`, "top-right");
rt.addButton(() => paused ? "||" : ">", (e) => paused = !paused);
const { width, height } = Z();
nt.create();
const env = new Env(width, height);
const game = new pt("Space Invader", env);
