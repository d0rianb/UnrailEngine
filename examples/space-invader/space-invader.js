...
class k {
  ...
  static onKeyDown(t2, e2) {
    t2 instanceof Array ? t2.forEach((t3) => K.addEvent(new k(t3, e2, 1))) : K.addEvent(new k(t2, e2, 1));
  }
  static onKeyPressed(t2, e2) {
    t2 instanceof Array ? t2.forEach((t3) => K.addEvent(new k(t3, e2, 0))) : K.addEvent(new k(t2, e2, 0));
  }
  static onAnyKeyReleased(t2) {
    K.addEvent(new k("keyup", t2, 3));
  }
  static onClick(t2) {
    k.onMouseClick(t2);
  }
...
class at {
  constructor(t2, e2) {
    ...
    this.id = nt++, this.image = new Image(), this.image.src = t2, this.image.onload = () => this.isLoaded = true, this.size = new h(this.image.width, this.image.height), this.rotation = (e2 == null ? void 0 : e2.rotation) || 0, this.offset = (e2 == null ? void 0 : e2.offset) || m, this.scale = (e2 == null ? void 0 : e2.scale) || p;
  }
...
class xt extends yt {
  constructor(t2, e2, s2 = 5, i2, n2) {
    super(e2.x, e2.y), this.radius = 2, this.id = t2, this.pos = e2.clone(), this.angle = i2 && i2 != "random" ? i2 % 2 * Math.PI : Math.PI / 2 + 2 * Math.random() * Math.PI, this.velocity = new h(Math.random() * s2 * Math.cos(this.angle), Math.random() * s2 * Math.sin(this.angle)), this.color = n2 || "transparent", this.opacity = y(100, 255 * Math.random(), 255);
  }
  update() {
    this.velocity.y += 0.01, this.pos = this.pos.add(this.velocity), this.opacity -= 2;
  }
  render() {
    mt.renderer.circle(this.pos.x, this.pos.y, this.radius, { fillStyle: this.color, lineWidth: 1, globalAlpha: this.opacity / 255 });
  }
}
class Gt {
  constructor(t2, e2, s2, i2) {
...
class Enemy extends yt {
  constructor(x, y) {
    ...
    this.width = 80;
    this.height = 50;
    this.texture = new at("resources/assets/InvaderA2.png");
  }
  isDead() {
    if (!this.alive)
      return;
  ...
  render() {
    if (!this.alive)
      return;
    it.rectSprite(this.x, this.y, this.width, this.height, this.texture);
  }
}
class Player extends bt {
  constructor(x, y) {
    ...
    this.health = 100;
    this.alive = true;
    this.texture = new at("resources/assets/space-invader-player.png");
  }
  isDead() {
    this.alive = false;
  }
  ...
  render() {
    it.rectSprite(this.x, this.y, 60, 30, this.texture);
  }
}
class Shot extends yt {
  constructor(x, y) {
    super(x, y);
    this.width = 2;
    this.height = 5;
  ...
  render() {
    it.rect(this.x, this.y, this.width, this.height, { lineWidth: 4, strokeStyle: "red" });
  }
}
class Env extends pt {
  ...
  hit(shot, enemy) {
    ...
    const PG = new Gt(50, new h(shot.x, shot.y), 700, () => {
...
const game = new mt("Space Invader", env);
